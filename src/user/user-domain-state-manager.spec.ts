...
import { ComponentState, ComponentStateInfo, ManagedStatefulComponent, ManagedStatefulComponentMixin } from '../libraries/managed-stateful-component';
import { MergedStreamLogger, StreamLoggableMixin, StreamLogger } from '../libraries/stream-loggable';

import UserDataService from '../user/services/user-data.service';
import UserDomainStateManager from '../user/user-domain-state-manager';
...
class MockUserDataService extends StreamLoggableMixin(ManagedStatefulComponentMixin(class {})) implements ManagedStatefulComponent {}
class MockUserRepository extends StreamLoggableMixin(ManagedStatefulComponentMixin(class {})) implements ManagedStatefulComponent {}

describe('UserDomainStateManager', () => {
	let manager: UserDomainStateManager;
	...
	beforeEach(async () => {
		...
		manager = module.get<UserDomainStateManager>(UserDomainStateManager);
	});
	...
	describe('Management API', () => {
			...
			beforeEach(async () => {
				...
				manager['msc_zh7y_stateSubject'].next({name: 'UserDataService', state: ComponentState.UNINITIALIZED, updatedOn: new Date()}); // set state to UNINITIALIZED
			});		
