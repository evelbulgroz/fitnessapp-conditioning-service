...
export class DomainHierarchyWirer {
		...
		protected buildHierarchy(
				...
				const { pathToManager, pathToChildren } = this.extractPathMappings(
						managers, pathExtractor, pathSeparator
				);
		...
		protected extractPathMappings(
				...
				for (const manager of managers) {
						const path = pathExtractor(manager).toLowerCase(); // Normalize to lowercase
						...
						this.registerParentChildRelationship(path, pathSeparator, pathToChildren);
				}
