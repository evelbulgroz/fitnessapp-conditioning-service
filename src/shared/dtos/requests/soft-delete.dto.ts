...
/**
 * DTO for sanitizing a single softDelete value in a response
 * 
 * @remark Defers validation to the BooleanDTO base class, which handles boolean values.
 * @remark Ignore the private '_includeDeleted' property autogenerated by the @IsDefined() decorator:
 *  only ever access the value via the 'includeDeleted' or 'value' properties, which point to the same value.
 * 
 * @todo Remove APIProperty decorator if/when @evelbulgroz/sanitizer-decorator adds support for Swagger
 */
export class SoftDeleteDTO extends BooleanDTO {
	...
	@ApiProperty({
		type: String,
		description: 'Soft delete value (boolean). Must conform to validation in BooleanDTO base class.',
		required: true,
	})
