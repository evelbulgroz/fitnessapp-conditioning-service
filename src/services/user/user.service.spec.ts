	...
	describe('Public API', () => {
		...
		describe('fetchByUserId()', () => {
			...
			it('throws error if caller is not user microservice', async () => {
				...
				// act
				const result = service.fetchByUserId(invalidContext, randomUserId);

				// assert
				await expect(result).rejects.toThrow('User invalid not authorized to access UserService.fetchByUserId');
			});
			...
			it('throws error if caller is not admin', async () => {
				...
				// assert
				await expect(result).rejects.toThrow('User fitnessapp-user-service not authorized to access UserService.fetchByUserId');
			});
			...
			it('throws error if user id is invalid', async () => {
				...
				// act
				const result = service.fetchByUserId(userContext, randomUserId);

				// assert
				await expect(result).rejects.toThrow("UserService.fetchByUserId requires a valid user id, got: null");
			});
		});
