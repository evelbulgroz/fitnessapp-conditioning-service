...
// NOTE: Testing over Http to enable decorators and guards without having to do a ton of additional setup/mokcing
	...
	*/
	...
	describe(`'logs' endpoint`, () => {
		...
		beforeEach(() => {
			...
			query = new LogsQuery(queryDto);

			logsSpy = jest.spyOn(conditioningDataService, 'conditioningLogs')
				.mockImplementation((query: any, ctx: any): any => { // todo: refactor service to use LogsQuery and UserContext
		...
		it('provides a collection of conditioning logs matching a query', async () => {
			...
			// act
			const response = await lastValueFrom(http.get(url, { params: queryDto, headers }));
			
			// assert
			expect(logsSpy).toHaveBeenCalledTimes(1);
			expect(logsSpy).toHaveBeenCalledWith(query, userContext);
			expect(response?.data).toBeDefined();
		...
		it('fails if access token is missing', async () => {
			// arrange
			const response$ = http.get(url, { params: queryDto });
		...
		it('fails if access token is invalid', async () => {
			// arrange
			const invalidHeaders = { Authorization: `Bearer invalid` };
			const response$ = http.get(url, { params: queryDto, headers: invalidHeaders });
		...
		it('fails if query is present but has invalid data', async () => {
			// arrange
			queryDto.start = 'invalid'; // invalid date
			const response$ = http.get(url, { params: queryDto, headers });
		...
		it('fails if query is present but has non-whitelisted properties', async () => {
			// arrange
			(queryDto as any).invalid = 'invalid'; // invalid property
			const response$ = http.get(url, { params: queryDto, headers });
